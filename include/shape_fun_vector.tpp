...
template <int dim>
inline
ShapeFunctionVector<dim>::ShapeFunctionVector (const FiniteElement<dim> &fe,
			typename Triangulation<dim>::cell_iterator &cell,
			bool verbose)
:
Function<dim> (dim),
...
template <int dim>
inline void
ShapeFunctionVector<dim>::set_current_cell (const typename Triangulation<dim>::cell_iterator &cell)
{
	current_cell_ptr = &cell;
}
...
template <int dim>
inline void
ShapeFunctionVector<dim>::vector_value(const Point<dim>   &p, Vector<double> &value) const
{
	...
	{
		value[i] = fe_values.shape_value_component (shape_fun_index, /* q_index */ 0, i);
	}
}
...
template <int dim>
inline void
ShapeFunctionVector<dim>::vector_value_list(const std::vector<Point<dim> > &points,
		 	 	 	 	 	 	 std::vector<Vector<double>>    &values) const
{
