...
template<int dim>
ShapeFunctionConcatinateVector<dim>::ShapeFunctionConcatinateVector (const Function<dim> &function1,
												const Function<dim> &function2)
:
...
template <int dim>
inline void
ShapeFunctionConcatinateVector<dim>::vector_value(const Point<dim>   &p, Vector<double> &value) const
{
	Vector<double> value1(function_ptr1->n_components);
	function_ptr1->vector_value (p, value1);
...
//template<int dim>
//inline void
//ShapeFunctionConcatinateVector<dim>::vector_value_list (const std::vector<Point<dim> > &points,
//		 	 	 	 	 	 	 	 	 	 	 	 std::vector<Vector<double>>    &values) const
//{
...
//	{
//		for (unsigned int j = 0; j < function_ptr1->n_components; ++j)
//			values.at(i)(j) = function_ptr1->value (points.at(i));;
//		for (unsigned int j = 0; j < function_ptr2->n_components; ++j)
//			values.at(i)(j) = function_ptr2->value (points.at(i));
//	}
//}
