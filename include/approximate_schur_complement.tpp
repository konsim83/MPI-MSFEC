#ifndef INCLUDE_APPROXIMATE_SCHUR_COMPLEMENT_TPP_
#define INCLUDE_APPROXIMATE_SCHUR_COMPLEMENT_TPP_
...
template <typename BlockMatrixType, typename VectorType, typename PreconditionerType>
ApproximateSchurComplement<BlockMatrixType, VectorType, PreconditionerType>::ApproximateSchurComplement(
...
template <typename BlockMatrixType, typename VectorType, typename PreconditionerType>
void
ApproximateSchurComplement<BlockMatrixType, VectorType, PreconditionerType>::vmult(
		VectorType       &dst,
		const VectorType &src) const
{
...
public:
	ApproximateSchurComplementMPI (const BlockMatrixType &system_matrix,
...
template <typename BlockMatrixType, typename VectorType, typename PreconditionerType>
ApproximateSchurComplementMPI<BlockMatrixType, VectorType, PreconditionerType>::ApproximateSchurComplementMPI(
		const BlockMatrixType	&system_matrix,
		const std::vector<IndexSet> &owned_partitioning,
		MPI_Comm mpi_communicator)
:
...
template <typename BlockMatrixType, typename VectorType, typename PreconditionerType>
void
ApproximateSchurComplementMPI<BlockMatrixType, VectorType, PreconditionerType>::vmult(
		VectorType       &dst,
		const VectorType &src) const
{
	system_matrix->block(0,1).vmult (tmp1, src);
...
#endif /* INCLUDE_APPROXIMATE_SCHUR_COMPLEMENT_TPP_ */
