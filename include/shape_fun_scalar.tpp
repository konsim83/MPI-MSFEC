...
template <int dim>
inline
ShapeFunctionScalar<dim>::ShapeFunctionScalar (const FiniteElement<dim> &fe,
		typename Triangulation<dim>::active_cell_iterator &cell,
		bool verbose)
:
Function<dim> (1),
...
template <int dim>
inline void
ShapeFunctionScalar<dim>::set_current_cell (const typename Triangulation<dim>::active_cell_iterator &cell)
{
	current_cell_ptr = &cell;
}
        						...
        						const unsigned int  /*component*/) const
{
	...
	FEValues<dim>		fe_values(mapping, *fe_ptr, fake_quadrature,
