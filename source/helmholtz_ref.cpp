...
//	preconditioner_matrix = 0;
	...
	FullMatrix<double>   local_matrix_precon (dofs_per_cell, dofs_per_cell);
				...
				for (unsigned int i=0; i<dofs_per_cell; ++i)
				{
					...
					for (unsigned int j=0; j<dofs_per_cell; ++j)
					{
						...
						local_matrix(i,j) += (tau_i * a_inverse_values[q] * sigma_j /* block (0,0) */
																...
																* fe_values.JxW(q);
//						local_matrix_precon(i, j) += 1.0 /  div_v_i * b_values[q] * div_u_j * fe_values.JxW(q);
					} // end for ++j
...
//			constraints.distribute_local_to_global(local_matrix_precon,
//													 local_dof_indices,
...
void
NedRTStd::solve_iterative_u_sigma ()
{
...
void
NedRTStd::solve_iterative_sigma_u ()
{
	inner_schur_preconditioner = std::make_shared<typename LinearSolvers::InnerPreconditioner<3>::type>();
	...
	inner_schur_preconditioner->initialize(system_matrix.block(1, 1), data);

	const LinearSolvers::InverseMatrix<LA::MPI::SparseMatrix,
		typename LinearSolvers::InnerPreconditioner<3>::type>
			block_inverse (system_matrix.block(1,1),
					*inner_schur_preconditioner );
	...
	// tmp of size block(1)
	LA::MPI::Vector	tmp(owned_partitioning[1],
							mpi_communicator);
	...
	// Compute schur_rhs = -f + B^T*C^{-1}*g
	LA::MPI::Vector schur_rhs (owned_partitioning[0],
//								relevant_partitioning[1],
								mpi_communicator);
	block_inverse.vmult (tmp, system_rhs.block(1));
	system_matrix.block(0,1).vmult (schur_rhs, tmp);
	schur_rhs -= system_rhs.block(0);
	{
		TimerOutput::Scope t(computing_timer, "Schur complement solver (for sigma)");
...
//													PETScWrappers::PreconditionSOR>
//														preconditioner (system_matrix, owned_partitioning, mpi_communicator);
//		PETScWrappers::PreconditionBoomerAMG precon_invC;
//		typename PETScWrappers::PreconditionBoomerAMG::AdditionalData dataC;
//		dataC.symmetric_operator = true; // Only for AMG
//		precon_invC.initialize(system_matrix.block(1, 1), dataC);
//
//		const LinearSolvers::InverseMatrix<LA::MPI::SparseMatrix,
//						typename LinearSolvers::InnerPreconditioner<3>::type>
//						preconditioner_invC (system_matrix.block(1,1),
//											precon_invC );
	#else
		typename TrilinosWrappers::PreconditionIdentity preconditioner;
	#endif
	...
	{
		TimerOutput::Scope t(computing_timer, "outer CG solver (for u)");
	...
	//	outer_solver.set_symmetric_mode(true);
		...
		// use computed u to solve for sigma
		system_matrix.block(1,0).vmult (tmp, distributed_solution.block(0));
		tmp *= -1;
		tmp += system_rhs.block(1);

		// Solve for sigma
	//	outer_solver.solve(system_matrix.block(0,0), distributed_solution.block(0), tmp);
		block_inverse.vmult (distributed_solution.block(1), tmp);
...
//	else
//	{
		solve_iterative_u_sigma (); // Schur complement for A
//		solve_iterative_sigma_u (); // Schur complement for C
//	}
//
	{
