...
namespace LaplaceProblem
{
  ...
  NedRTStd::assemble_system()
  {
    ...
    QGauss<2> face_quadrature_formula(parameters.degree + 5);
    ...
    if (parameters.use_exact_solution)
      right_hand_side.reset(new RightHandSideExactLin(parameter_filename));
    else
      right_hand_side.reset(new RightHandSideParsed());
    ...
    // Boundary values
    std::unique_ptr<ExactSolutionLin_B_div> minus_B_div_u;
    std::unique_ptr<ExactSolutionLin>       u;
    if (parameters.use_exact_solution)
      {
        minus_B_div_u.reset(new ExactSolutionLin_B_div(parameter_filename));
        u.reset(new ExactSolutionLin(parameter_filename));
      }
...
//	data.symmetric_operator = true; // Only for AMG
    ...
    schur_rhs -= system_rhs.block(1);
    {
      ...
      //																					LA::MPI::Vector,
      //																					typename
      //LinearSolvers::InnerPreconditioner<3>::type>, 									PreconditionIdentity>
  ...
  NedRTStd::write_exact_solution()
  {
    ...
    { // write sigma
      ...
      // Setup function
      ExactSolutionLin_A_curl exact_sigma(parameter_filename);
    ...
    { // write u
      ...
      // Setup function
      ExactSolutionLin exact_u(parameter_filename);
  ...
  NedRTStd::output_results() const
  {
    ...
    if (parameters.use_exact_solution)
      {
        postprocessor_exact.reset(
          new NedRT_PostProcessor(parameter_filename, "exact_"));
        data_out.add_data_vector(locally_relevant_exact_solution,
                                 *postprocessor_exact);
