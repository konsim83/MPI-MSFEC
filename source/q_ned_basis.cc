...
namespace LaplaceProblem
{
  ...
  QNedBasis::QNedBasis(
    const Parameters::QNed::ParametersMs &           parameters_ms,
  ...
  QNedBasis::solve_iterative(unsigned int n_basis)
  {
    ...
    Vector<double> tmp(system_rhs.block(0).size());
    {
...

      {
        ...
        //PreconditionJacobi<SparseMatrix<double>>; 			using ApproxSchurPrecon =
  ...
  void
    QNedBasis::write_exact_solution_in_cell()
    {
      ...
      { // write sigma
        ...
        // Setup function
        ExactSolutionLin_B_div exact_sigma(parameter_filename);
	  ...
	  DataOut<3> data_out;
	      ...
	      std::vector<DataComponentInterpretation::DataComponentInterpretation>
	        data_component_interpretation_exact(
	          1, DataComponentInterpretation::component_is_scalar);
		  data_component_interpretation_exact.emplace_back(DataComponentInterpretation::component_is_part_of_vector);
		  data_component_interpretation_exact.emplace_back(DataComponentInterpretation::component_is_part_of_vector);
		  data_component_interpretation_exact.emplace_back(DataComponentInterpretation::component_is_part_of_vector);
	      ...
	      if (parameters.use_exact_solution)
	        {
	          postprocessor_exact.reset(
	            new QNed_PostProcessor(parameter_filename, "exact_"));
