	...
	std::vector<Vector<double>> &computed_quantities) const
{
	...
	a_inverse.value_list (inputs.evaluation_points,
			a_inverse_values);
	b.value_list(inputs.evaluation_points,
			b_values);
	...
	for (unsigned int q = 0; q < n_quadrature_points; ++q)
	{
		for (unsigned int d = 0; d < 3; ++d)
		{
			...
			for (unsigned int i = 0; i < 3; ++i)
				computed_quantities[q](d) += a_inverse_values[q][d][i] * inputs.solution_values[q][i];
		}
